...
export const slugOpcional = z.union([
  z.literal(''),
  z
    .string()
...
const ROL_VALIDO = z.enum(['admin', 'organizador', 'usuario'], {
  message: 'Selecciona un rol válido',
});
const ESTADO_VALIDO = z.enum(['activo', 'suspendido', 'inactivo'], {
...
export const crearUsuarioSchema = z.object({
  ...
  rol: ROL_VALIDO.default('usuario'),
  estado: ESTADO_VALIDO.default('activo'),
  slug: slugOpcional,
}).refine((d) => d.password === d.confirmPassword, {
  message: 'Las contraseñas no coinciden',
  path: ['confirmPassword'],
...
export const actualizarUsuarioSchema = z.object({
  ...
  nombre: z.union([z.literal(''), nombreSchema]),
  apellido: z.union([z.literal(''), apellidoBase]),
  telefono: telefonoOpcional,
  cedula: cedulaOpcional,
  rol: ROL_VALIDO.optional(),
  estado: ESTADO_VALIDO.optional(),
}).refine(
  (d) =>
    !d.password ||
    !d.confirmPassword ||
...
export const categoriaSchema = z.object({
  ...
  descripcion: z
    ...
    .optional()
    .or(z.literal('')),
  iconoUrl: urlOpcional.optional(),
  colorHex: z
    .string()
    .max(7, 'Formato de color inválido')
