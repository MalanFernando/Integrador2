...
function parseInformacionPago(
  data: InformacionPago | null | undefined,
...
export function EventoForm({
  ...
  const [showUbicacionOnline, setShowUbicacionOnline] = useState(
    initialData
      ? (initialData.online ? Boolean(initialData.ubicacionId) : true)
      : true,
  );
  ...
  const localidadesFieldArray = useFieldArray<
    EventoFormValues,
    'localidades'
  >({
    control,
  ...
  return (
    ...
    >
      ...
      {urlImportada && (
        <div className="flex items-center gap-3 rounded-md border border-blue-800 bg-blue-950/50 p-4 text-sm text-blue-300">
          <CheckCircle2 className="h-5 w-5 shrink-0" />
          <div>
            <p className="font-medium">
              Evento importado desde URL. Revisa y corrige los datos antes de
              publicar.
            </p>
            {urlImportada && (
              <a
      ...
      <SectionCard title="Datos generales">
        <div>
          <p className="mb-2 text-sm font-medium text-white">
            Imagen de portada <span className="text-white/50">({imagenes.length}/3)</span>
          </p>
          <div className="flex flex-wrap gap-3">
            {imagenes.map((img, idx) => (
              <div
                key={img}
                className="relative h-24 w-24 overflow-hidden rounded-md border border-white/10"
              >
        ...
        <Field label="Descripción" required>
          <textarea
            ...
            className={cn(inputClasses(Boolean(errors.descripcion)), 'resize-y')}
            {...register('descripcion')}
          />
          {errors.descripcion && (
            <p className="mt-1 text-xs text-[#C04C4C]">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Provincia" required>
                ...
                >
                  ...
                  {provincias.map((p) => (
                    <option key={p.id} value={String(p.id)} className="bg-black">
                      {p.nombre}
                    </option>
                  ))}
                </select>
        ...
        <div className="space-y-1">
          ...
          {sumaLabel && sumaAforos !== Number(aforoActual || 0) && (
            <button
              ...
              onClick={() =>
                setValue('aforo', String(sumaAforos), { shouldValidate: true })
              }
            >
      ...
      {/* Información de pago */}
      <SectionCard title="Información de pago y venta de tickets">
        <label className="flex items-center gap-3 text-sm font-medium text-white">
          <input
            type="checkbox"
            className="h-4 w-4 rounded border-white/30"
        ...
        {esGratuito ? (
          ...
          <div className="space-y-4">
            {errors.esGratuito && (
              <p className="text-sm text-red-400">{errors.esGratuito.message}</p>
            )}
            <div className="grid gap-4 sm:grid-cols-2">
              <Input
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Controller
                control={control}
                name="informacionPago.tipoCuenta"
                render={({ field }) => (
                  <SelectField
                    label="Tipo de cuenta"
                    value={field.value ?? 'ahorros'}
                    onChange={field.onChange}
                    options={[
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-3">
                  ...
                  />
                  <label className="inline-flex cursor-pointer items-center gap-2 rounded-md border border-white/50 px-3 py-1.5 text-sm text-white hover:bg-white/10">
                    {uploadingCedula ? (
          ...
          >
            ...
            <textarea
              ...
              className={cn(
                inputClasses(
                  Boolean(
                    errors.preguntasFrecuentes?.[index]?.respuesta?.message,
                  ),
                ),
                'resize-y',
              )}
      ...
      <div className="flex items-center justify-end gap-3 pb-4">
        ...
        <Button type="submit" disabled={saving} className="gap-2">
          ...
          {mode === 'editar' ? 'Guardar cambios' : 'Publicar evento'}
        </Button>
      </div>
    </form>
  );
