...
const reviews = [
  {
    ...
    texto: 'El evento estuvo regular, la organización dejó mucho que desear. Los baños estaban sucios y el sonido no era el mejor.',
    tipo: 'reportado',
  },
  {
    id: '2',
    ...
    texto: 'Excelente experiencia, la comida deliciosa y el ambiente increíble. Muy recomendado para pasar en familia.',
    tipo: 'normal',
  },
  {
    id: '3',
    ...
    texto: 'Muy mal servicio, nos hicieron esperar demasiado y el show no valió la pena. No volvería.',
    tipo: 'advertencia',
  },
  {
    id: '4',
    ...
    texto: 'Buena exposición aunque esperaba más variedad de obras. El lugar es bonito y bien ubicado.',
    tipo: 'normal',
  },
];

const estadoOptions = ['Todos', 'Reportado', 'En revisión', 'Aprobado'];
const puntuacionOptions = ['Todas', '5 estrellas', '4 estrellas', '3 estrellas', '2 estrellas', '1 estrella'];
...
export default function AdminResenasPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-clash text-2xl font-semibold text-white">Moderación de reseñas</h1>
        <p className="text-sm text-white/50 mt-1">Revisión y gestión de comentarios publicados por usuarios</p>
      </div>
        ...
        <div className="relative">
          <select className="appearance-none bg-white/5 text-white text-sm rounded px-3 py-2 pr-8 border border-white/10 outline-none">
            {puntuacionOptions.map((o) => (
              <option key={o} value={o} className="bg-black">{o === 'Todas' ? 'Puntuación' : o}</option>
            ))}
          </select>
          <ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 h-4 w-4 text-white/50 pointer-events-none" />
        </div>
          ...
          return (
            <div
              ...
              className={`border rounded-lg ${
                isReportado ? 'border-red-500/75' : isAdvertencia ? 'border-yellow-500/75' : 'border-white/10'
              }`}
            >
              <div className="p-5">
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex items-center gap-1">
                      <Star className={`h-4 w-4 ${r.rating >= 4 ? 'text-[#F4A261] fill-[#F4A261]' : 'text-white/30'}`} />
                ...
                <p className="text-xs text-white/50 mb-3">Evento: <span className="text-white/70">{r.evento}</span></p>
                ...
                {isReportado && (
                  <p className="text-xs text-red-400 mt-2">Motivo: Contenido inapropiado y lenguaje ofensivo</p>
                )}
              </div>
            </div>
          );
